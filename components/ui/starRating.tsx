...
const yellowStarSvg = <svg aria-hidden="true" className="h-5 w-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"
...
const grayStarSvg = <svg aria-hidden="true" className="h-5 w-5 text-gray-300 dark:text-gray-500" fill="currentColor"
...
function makeStars(count: number): ReactNode[] {
  ...
  for (let i = 0; i < count; i++) {
    stars.push(yellowStarSvg)
  }
  for (let i = 0; i < 5 - count; i++) {
    stars.push(grayStarSvg)
  }
  return stars
}
